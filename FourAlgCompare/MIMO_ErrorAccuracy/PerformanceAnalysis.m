...
figure;
%subplot(221)
plot(k,RealState(3,:),'b-',k,FEKF_X_esti(3,:),'r--','linewidth',LineWidth);
...
figure;
%subplot(222)
plot(k,RealState(2,:),'b-',k,FCDKF_X_esti(2,:),'r--','linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
axis normal
axis([0 50 0 1])
ylabel('x','FontSize',FontSize)
...
figure;
%subplot(223)
plot(k,RealState(3,:),'b-',k,FUKF_X_esti(3,:),'r--','linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
% axis normal
% axis([0 50 0 1])
ylabel('x','FontSize',FontSize)
