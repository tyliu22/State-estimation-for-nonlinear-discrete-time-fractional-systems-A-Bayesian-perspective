...
%
%   备注：图1：估计状态与真实状态比较图
%         图2：与 PerfAnaFPF.m 的区别主要是 y轴坐标改为对数表示
%         图3：表示ISE指标下的性能分析图
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% FPF_X_esti_500 = log2(FPF_X_esti_500);
% FPF_X_esti_250 = log2(FPF_X_esti_250);
% FPF_X_esti_100 = log2(FPF_X_esti_100);
...
ISE_FPF_X_esti_100 = zeros(3,50);
ISE_FPF_X_esti_250 = zeros(3,50);
ISE_FPF_X_esti_500 = zeros(3,50);

    ISE_FPF_X_esti_100(:,1) = Error_FPF_X_esti_100(:,1);
    for i = 2:1:50
        ISE_FPF_X_esti_100(:,i) = ISE_FPF_X_esti_100(:,i-1) + Error_FPF_X_esti_100(:,i);
    end
    
    ISE_FPF_X_esti_250(:,1) = Error_FPF_X_esti_250(:,1);
    for i = 2:1:50
        ISE_FPF_X_esti_250(:,i) = ISE_FPF_X_esti_250(:,i-1) + Error_FPF_X_esti_250(:,i);
    end
    
    ISE_FPF_X_esti_500(:,1) = Error_FPF_X_esti_500(:,1);
    for i = 2:1:50
        ISE_FPF_X_esti_500(:,i) = ISE_FPF_X_esti_500(:,i-1) + Error_FPF_X_esti_500(:,i);
    end
...
figure;
subplot(311)
semilogy(k,(Error_FPF_X_esti_100(1,:)),'b-',k,(Error_FPF_X_esti_250(1,:)),'r--',k,(Error_FPF_X_esti_500(1,:)),':c', ...
    'linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
axis normal
%axis([0 25])
...
subplot(312)
semilogy(k,Error_FPF_X_esti_100(2,:),'b-',k,Error_FPF_X_esti_250(2,:),'r--',k,Error_FPF_X_esti_500(2,:),':c', ...
    'linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
% axis normal
% axis([0 25 0 4])
...
subplot(313)
semilogy(k,Error_FPF_X_esti_100(3,:),'b-',k,Error_FPF_X_esti_250(3,:),'r--',k,Error_FPF_X_esti_500(3,:),':c', ...
    'linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
% axis normal
% axis([0 25 0 0.5])
...
figure;
subplot(311)
plot(k,(ISE_FPF_X_esti_100(1,:)),'b-',k,(ISE_FPF_X_esti_250(1,:)),'r--',k,(ISE_FPF_X_esti_500(1,:)),':c', ...
    'linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
axis normal
%axis([0 25])
% axis([0 25 0 0.3])
ylabel('ISE($x_1$)','FontSize',FontSize,'Interpreter','latex')
xlabel('$k$','FontSize',FontSize,'Interpreter','latex')
% 设置坐标轴刻度字体名称，大小
set(gcf,'Position',[200 200 400 300]); 
set(gca,'FontName','Helvetica','FontSize',FontSize)
...
subplot(312)
plot(k,ISE_FPF_X_esti_100(2,:),'b-',k,ISE_FPF_X_esti_250(2,:),'r--',k,ISE_FPF_X_esti_500(2,:),':c', ...
    'linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
% axis normal
% axis([0 25 0 4])
ylabel('ISE($x_2$)','FontSize',FontSize,'Interpreter','latex')
xlabel('$k$','FontSize',FontSize,'Interpreter','latex')
% 设置坐标轴刻度字体名称，大小
set(gcf,'Position',[200 200 400 300]); 
set(gca,'FontName','Helvetica','FontSize',FontSize)
...
subplot(313)
plot(k,ISE_FPF_X_esti_100(3,:),'b-',k,ISE_FPF_X_esti_250(3,:),'r--',k,ISE_FPF_X_esti_500(3,:),':c', ...
    'linewidth',LineWidth);
% axis([xmin xmax ymin ymax])  设置坐标轴在指定的区间
% axis normal
% axis([0 25 0 0.5])
ylabel('ISE($x_3$)','FontSize',FontSize,'Interpreter','latex')
xlabel('$k$','FontSize',FontSize,'Interpreter','latex')
% 设置坐标轴刻度字体名称，大小
set(gcf,'Position',[200 200 400 300]); 
set(gca,'FontName','Helvetica','FontSize',FontSize)
